...
public class RowsTest {
    ...
    private Rows nullCursorGetString(Filter filter) {
    ...
    @Test
    public void testFolder2DifferentSong() {
    ...
    @Test
    public void testArtist2DifferentSong() {
    ...
    @Test
    public void testTree2DifferentSong() {
    ...
    private Rows mergeNameCase(Filter filter) {
    ...
    private Rows normalList(Filter filter) {
        Log.d("RowsTest", "== testMergeNameCase ==");
    ...
    @Test
    public void testTreeNormalList() {
    ...
    private Rows initRows(String[][] data, Filter filter) {
        ...
        TestContentProvider provider = new TestContentProvider(null, data);
        resolver.addProvider(MediaStore.AUTHORITY, provider);
        Rows rows = new Rows(null, resolver, new ParametersStub(), null);
        rows.setFilter(filter);
        rows.init();
        return rows;
    }

    private void checkRowSize(Rows rows, int size) {
        ArrayList<Row> arrayRowUnfold = (ArrayList<Row>) getField(rows, "rowsUnfolded");
        printRowArray(arrayRowUnfold);
        assertEquals(arrayRowUnfold.size(), size);
    }

    private void checkRowsText(Rows rows, String[] names) {
