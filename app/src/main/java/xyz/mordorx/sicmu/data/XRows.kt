...
object XRows {
    ...
    public var rowsFolded = MutableStateFlow<List<Row>>(emptyList())
        private set
    ...
    public fun initRowsFromMediaStore() = CoroutineScope(Dispatchers.IO).launch {
        ...
        val groups = folders.sortedBy { str -> str }.mapNotNull {
            ...
            // TODO: Add SortedList() class/type that finds upper and lower bound of path match, using binary search.
            val songsContained = rows
                .filter { r -> (r as XRowSong).path.startsWith(fullPath) }

            val songCount = songsContained.size
            val playtimeMs = songsContained.sumOf { song -> (song as XRowSong).durationMs }
        ...
        val dur = finishMs - startTime
        Log.d("XRows-Finale-Time", "scan took $dur Ms")
